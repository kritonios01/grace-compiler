...
let digit          = ['0'-'9']
(* let common_chars   = [^ '\'' '\"' '\\'] *) (* edw exoume thema *)
let escseq         = ['\n' '\t' '\r' '\000' '\\' '\'' '\"'] | ('\\' 'x' (digit | ['A'-'F']) (digit | ['A'-'F'])) | _
...
rule lexer = parse
  ...
  (* char constants *) (*kai edw antistoixa thema *)
  (*| '\'' T_constc '\'' { T_constc }
  | '\"' comc+ '\"'    { T_consts }*)
  | '\'' _ '\''     { T_constc }
  | '\"' _+ '\"'    { T_consts }
...
and comment = shortest
  ...
  (*| [^'$']    {comment lexbuf}*)
  | _      {Printf.eprintf "Error on single line comment: Dual $\n"; lexer lexbuf}
